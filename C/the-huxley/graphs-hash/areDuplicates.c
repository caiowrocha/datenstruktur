...
bool contains(TIPOCHAVE value, TIPOCHAVE set[], int size) {
...
void setAdd(TIPOCHAVE value, TIPOCHAVE set[], int *size) {
     set[*size] = value;
     (*size)++;
};

bool areDuplicates(ARVORE_BINARIA *tree, TIPOCHAVE set[], int *size) {
  	if(vazia(tree)) {
    	return false;
    }
  
  	if(contains(tree->item.chave, set, *size)) {
       return true;
    };
  
    setAdd(tree->item.chave, set, size);
  
  	bool left = areDuplicates(tree->esq, set, size);
    bool right = areDuplicates(tree->dir, set, size);
...
return areDuplicates(arvore, set, &size);
