...
if(newNode->item.chave == l->cabeca->item.chave) return false;

if(newNode->item.chave < l->cabeca->item.chave) {
 	newNode->prox = l->cabeca;
  	l->cabeca = newNode;
  	return true;
};
...
while(helper->prox != NULL) {
  if(newNode->item.chave < helper->prox->item.chave) {
  	newNode->prox = helper->prox;
    helper->prox = newNode;
    return true;
  }
  if(newNode->item.chave == helper->prox->item.chave) return false;
  helper = helper->prox;
}
helper->prox = newNode;
return true;
